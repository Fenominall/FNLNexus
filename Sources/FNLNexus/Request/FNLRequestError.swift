...
public enum FNLRequestError: Error, CustomDebugStringConvertible, Equatable {
...
extension FNLRequestError {
    init(fromURLError urlError: URLError) {
            switch urlError.code {
            case .notConnectedToInternet:
                self = .noConnection
            case .cancelled:
                self = .unknown("Request cancelled")
            case .timedOut:
                self = .timeout
